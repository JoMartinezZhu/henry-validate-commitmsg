...
var error = function() {
    ...
    console[config.warnOnFail ? 'warn' : 'error'](
        `${chalk.red('INVALID COMMIT MSG: ' + util.format.apply(null, arguments))}`
    );
};
...
function validateScope(isValid, scope) {
    ...
    function validateIndividualScope(item) {
        ...
        for (
            var allowedScopesIndex = 0;
            allowedScopesIndex < allowedScopes.length;
            allowedScopesIndex++
        ) {
            if (new RegExp(allowedScopes[allowedScopesIndex]).test(item)) {
                isValid = true;
                // regFlag = true;
                break;
            } else {
